 ...
 */
add_action('widgets_init', 'rt_tutor_course_load_widgets');

function rt_tutor_course_load_widgets()
{
    register_widget('RT_Tutor_Course_Widget');
}
 ...
 *
 */
class RT_Tutor_Course_Widget extends WP_Widget
{
    /**
     * Widget setup.
     */
    function __construct()
    {
        ...
        $widget_ops = [
            ...
            'description' => esc_html__('Display Tutor recent course by categories (or related)', 'gostudy-core')
        ];

        /* Create the widget. */
        parent::__construct('rt-tutor-course', esc_html__('(GOSTUDY) Tutor Courses', 'gostudy-core'), $widget_ops);
    }
    ...
    function widget($args, $instance)
    {
        ...
        if ($recent_posts->have_posts()) :
            echo '<ul class="rt-tutor-course-widget recent-widget-', esc_attr($time_id), '">';
                    ...
                    echo '<li class="clearfix', ($img_url ? ' has_image' : ''), '">';
                        if ($img_url) {
                            echo '<a class="post__link" href="', esc_url(get_permalink()), '">';
                                echo '<div class="rt-tutor-course-image_wrapper">',
                                    '<img',
                        ...
                        echo '<div class="rt-tutor-course-content_wrapper">';
                            echo '<a class="post__link" href="', esc_url(get_permalink()), '">';
                            ...
                            if ($text) :
                                echo '<div class="rt-tutor-course-content">',
                                    esc_html($text),
                                '</div>';
                            endif;
                        echo '</div>';
    ...
    function form($instance)
    {
        ...
        $defaults = [
            'title' => esc_html__('Tutor Courses' , 'gostudy-core'),
            'num_posts' => 3,
            'categories' => 'all',
            'show_related'  => 'off',
            'show_image' => 'on',
        ...
        <p>
            ...
            <select id="<?php echo esc_attr($this->get_field_id('categories')); ?>" name="<?php echo esc_attr($this->get_field_name('categories')); ?>" class="widefat categories" style="width:100%;">
                ...
                <?php $categories = get_categories( 'hide_empty=0&depth=1&type=courses&taxonomy=course-category' ); ?>
