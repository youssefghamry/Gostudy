...
(function( $ ) {
    ...
    $( document ).ready(
        function() {
            // redux.field_objects.custom_typography.init();
        }
    );

    redux.field_objects.custom_typography.init = function( selector, skipCheck) {

        if ( !selector ) {
            selector = $( document ).find( ".redux-group-tab:visible" ).find( '.redux-container-custom_typography:visible' );
        }
        $( selector ).each(
            function() {
                var el = $( this );
                    ...
                    function() {
                        ...
                        $( this ).find( '.redux-typography-container' ).each(
                            function() {
                                ...
                                redux.field_objects.custom_typography.select( family, true );
                        ...
                        $( this ).find( ".redux-typography-family" ).select2(
                            ...
                            "select2-opening", function( e ) {
                                ...
                                // Get field ID
                                var thisID = $( this ).parents( '.redux-container-custom_typography:first' ).attr( 'data-id' );
                        ...
                        if ( !xx.hasClass( 'redux-typography-family' ) ) {
                            ...
                            el.find( ".redux-typography-style-multi" ).select2( default_params );
                        }
    ...
    redux.field_objects.custom_typography.select = function( selector, skipCheck ) {
        var mainID;
        
        // Main id for selected field
        mainID = $( selector ).parents( '.redux-container-custom_typography:first' ).attr( 'data-id' );
        if (mainID === undefined) {
            mainID = $(selector).attr( 'data-id' );
        }
        
        var parent = $( selector ).parents( '.redux-container-custom_typography:first' );
        var data = [];
        ...
        var style_multi = $( '#' + mainID + ' select.redux-typography-style-multi' ).val();
        ...
        var style_multi_arr = '';
        if ( isSelecting === true ) {
            google = redux.field_objects.custom_typography.makeBool( selVals.object['data-google'] );
            $( '#' + mainID + ' .redux-typography-google-font' ).val( google );
        } else {
            google = redux.field_objects.custom_typography.makeBool( $( '#' + mainID + ' .redux-typography-google-font' ).val() ); // Check if font is a google font
        }
            ...
            if ( google === true ) {
                ...
                $.each(
                    details.variants, function( index, variant ) {
                        if ( variant.id === style || redux.field_objects.custom_typography.size( details.variants ) === 1 ) {
                ...
                $( '#' + mainID + ' .redux-typography-style-multi' ).select2( "destroy" );
                
                // Instert new HTML
                $( '#' + mainID + ' .redux-typography-style' ).html( html );
                $( '#' + mainID + ' .redux-typography-style-multi' ).html( html_multi );

                // Init select2
                $( '#' + mainID + ' .redux-typography-style' ).select2( default_params );
                $( '#' + mainID + ' .redux-typography-style-multi' ).select2( default_params );
                ...
                $.each(
                    details.subsets, function( index, subset ) {
                        if ( subset.id === script || redux.field_objects.custom_typography.size( details.subsets ) === 1 ) {
                            selected = ' selected="selected"';
                            script = subset.id;
                            $( '#' + mainID + ' input.typography-subsets' ).val( script );
                        } else {
                ...
                if ( details ) {
                    ...
                    $.each(
                        details, function( index, value ) {
                            ...
                            html += '<option value="' + index + '"' + selected_multi + '>' + value.replace(
                                '+', ' '
                            ) + '</option>';
                        }
                    );
                    ...
                    // Insert new HTML
                    $( '#' + mainID + ' .redux-typography-style' ).html( html );
                    $( '#' + mainID + ' .redux-typography-style-multi' ).html( html );
        ...
        if ( style_multi === '' || style_multi === undefined ) {
            $( '#' + mainID + ' .typography-font-weight-multi' ).val( '' );
        } else {
            $( '#' + mainID + ' .typography-font-weight-multi' ).val( style_multi );
        }
        ...
        if ( $( '#' + mainID ).hasClass( 'typography-initialized' ) ) {
            ...
            if ( color ) {
                ...
                $( '#' + mainID + ' .typography-preview' ).css(
                    'background-color', redux.field_objects.custom_typography.contrastColour( color )
                );
            }

            $( '#' + mainID + ' .typography-style .select2-chosen' ).text( $( '#' + mainID + ' .redux-typography-style option:selected' ).text() );
            $( '#' + mainID + ' .typography-multi .select2-chosen' ).text( $( '#' + mainID + ' .redux-typography-style-multi option:selected' ).text() );
